...
%DIF DEL ../preprintv1/stabledqmc.tex   Fri Apr 17 14:27:44 2020
%DIF ADD stabledqmc.tex                 Mon Apr 20 16:14:31 2020
...
\begin{figure}
	...
	\end{subfigure}
	\vspace{5pt}
	\caption[MyCaption]{\textbf{Numerical instabilities} due to finite machine precision \DIFdelbeginFL \DIFdelFL{(green) }\DIFdelendFL arising in the calculation of the time slice matrix chain product $B_M B_{M-1} \cdots B_1$ for model~\eqref{eq:model}. \DIFaddbeginFL \DIFaddFL{Different lines represent logarithmic singular values as observed in naive (green) and arbitrary precision computations (orange). }\DIFaddendFL The dashed line (grey) indicates the expected floating point \DIFdelbeginFL \DIFdelFL{precision}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{accuracy}\DIFaddendFL \protect\footnotemark.\DIFdelbeginFL \DIFdelFL{The arbitrary precision result (orange) is shown for comparison.}\DIFdelendFL }
...
\begin{figure}
	...
	\caption{\textbf{Comparison of matrix decompositions} to heal the numerical instabilities in the calculation of the time slice matrix chain product $B_M B_{M-1} \cdots B_1$ for model~\eqref{eq:model}. \DIFaddbeginFL \DIFaddFL{Different lines represent logarithmic singular values as observed in stabilized computations. }\DIFaddendFL The QR \DIFaddbeginFL \DIFaddFL{(orange) }\DIFaddendFL and Jacobi SVD singular values \DIFdelbeginFL \DIFdelFL{seem to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(yellow) }\DIFaddendFL lie on top of the exact \DIFdelbeginFL \DIFdelFL{ones }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{result (red) }\DIFaddendFL whereas \DIFaddbeginFL \DIFaddFL{both the }\DIFaddendFL regular SVD \DIFaddbeginFL \DIFaddFL{(green) }\DIFaddendFL and \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL divide-and-conquer SVD \DIFaddbeginFL \DIFaddFL{(purple) }\DIFaddendFL show large deviations at low temperatures $\beta \gtrsim 25$ ($\Delta \tau = 0.1$).}
	\label{fig:decomp_comparison_simple}
\end{figure}

Supplementing our general considerations above, we test the correctness of the matrix product stabilization procedure with respect to varying the concrete SVD and QR factorization algorithms. Fig.~\ref{fig:decomp_comparison_simple} shows the logarithmic singular values of the time slice matrix chain $B(\beta, 0)$ as a function of inverse temperature $\beta$ obtained from employing different matrix decompositions. Clearly, the accuracy of the computed singular values shows a strong dependence on the chosen factorization algorithm. While the results for the QR decomposition and Jacobi SVD seem to fall on top of the exact result, we observe large deviations for the conventional and D\&C SVD algorithms. This effect is particularly pronounced at low temperatues, $\beta \gtrsim 25$. The fact that small scales are lost \DIFdelbegin \DIFdel{while large scales remain relatively accurate }\DIFdelend \DIFaddbegin \DIFadd{in these SVD variants, while large ones are still correct, }\DIFaddend can be understood from LAPACK's SVD \DIFdelbegin \DIFdel{errorbounds \mbox{%DIFAUXCMD
\cite{errorbounds}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{error bounds \mbox{%DIFAUXCMD
\cite{errorbounds}}\hspace{0pt}%DIFAUXCMD
: The error is bounded relative to the largest singular value. Thus, large scales are computed to high relative accuracy and small ones may not be.
}\DIFaddend 
...
Finally, let us remark that the performance of any stabilization scheme is\DIFdelbegin \DIFdel{affected by the condition number of the involved imaginary time slice matrices and is therefore}\DIFdelend , in principle, model (parameter) dependent. \DIFdelbegin \DIFdel{We have not investigated this dependence in this manuscript, but have found qualitatively similar results for a }\DIFdelend \DIFaddbegin \DIFadd{While a systematic theoretical investigation is beyond the scope of this manuscript, we include a brief analysis of a }\DIFaddend spin-fermion model for antiferromagnetic metallic quantum criticality in \DIFdelbegin \DIFdel{Ref.~\mbox{%DIFAUXCMD
\cite{Bauer2020}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{App.~\ref{app:sdw}}\DIFaddend . We therefore believe that our major conclusions bear some universality and can serve as a useful guide.
...
\begin{figure}
	...
	\caption{\textbf{\DIFaddFL{Comparison of matrix decompositions}} \DIFaddFL{in the calculation of the time slice matrix chain product $B_M B_{M-1} \cdots B_1$ near a metallic antiferromagnetic quantum critical point (the spin-fermion model considered in Ref.~\mbox{%DIFAUXCMD
\cite{Bauer2020}}\hspace{0pt}%DIFAUXCMD
). Different lines represent a selection}\protect\footnotemark \DIFaddFL{of logarithmic singular values as observed in the stabilized computations. The divide-and-conquer SVD (purple) shows large deviations at low temperatures $\beta \gtrsim 8$ ($\Delta \tau = 0.1$) while the other matrix decompositions are in approximate agreement (on top of each other).}}
	\label{fig:decomp_comparison_simple_sdw}
\end{figure}

\footnotetext{\DIFadd{The four flavor (two spins, two bands) spin-fermion model with $L=10$ leads to 400 singular values $s_i$. For simplicity, Fig.~\ref{fig:decomp_comparison_simple} shows only 10 of those, namely $s_1, s_{41}, s_{81}, ..., s_{361}$.}}

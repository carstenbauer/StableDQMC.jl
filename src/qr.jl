...
Options for preallocation via keyword arguments:
  ...
  * `r = similar(F.U)`
"""
function udt_inv_one_plus_loh(F::UDT; r = similar(F.U), l = similar(F.U), internaluse = false)
  U,D,T = F
...
Options for preallocations via keyword arguments:
  ...
  * `mat2 = similar(A.U)`
"""
function udt_inv_sum_loh(A::UDT, B::UDT; mat2 = similar(A.U), internaluse = false)
    Ua, Da, Ta = A
    Ub, Db, Tb = B
